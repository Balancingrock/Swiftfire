...
final class ReadBlacklistReply: MacMessage {

    var json: VJson {
        let j = VJson()
        j[REPLY_NAME][SOURCE] &= source
        j[REPLY_NAME][LIST] &= blist.json
        return j
    }
    ...
    init?(json: VJson?) {
        ...
        guard let jlist = Blacklist(json: json|REPLY_NAME|LIST) else { return nil }
        self.source = jsource
        self.blist = jlist
    }
    ...
    init(source: String, list: Blacklist) {
        self.source = source
        self.blist = list
    }
}
