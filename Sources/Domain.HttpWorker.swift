...
extension Domain {
    ...
    func httpWorker(header: HttpHeader, body: Data, connection: HttpConnection, mutation: Mutation) -> Data? {
        ...
        switch blacklist.action(forAddress: connection.remoteAddress) {
            
        case nil: break // No blacklisting action required
            
        case Blacklist.Action.closeConnection?:
            
            log.atLevelNotice(id: connection.logId, source: #file.source(#function, #line), message: "Domain rejected blacklisted client \(connection.remoteAddress) by closing the connection")
            return nil


        case Blacklist.Action.send401Unauthorized?:
            
            log.atLevelNotice(id: connection.logId, source: #file.source(#function, #line), message: "Domain rejected blacklisted client \(connection.remoteAddress) with 401 reply")
            ...
            connection.transfer(reply)
            return nil


        case Blacklist.Action.send503ServiceUnavailable?:
            
            log.atLevelNotice(id: connection.logId, source: #file.source(#function, #line), message: "Domain rejected blacklisted client \(connection.remoteAddress) with 503 reply")
        ...
        if accessLogEnabled {
            ...
            accessLog?.record(time: Date(), ipAddress: connection.remoteAddress, url: url, operation: operation, version: version)
        }
        ...
        let response = HttpResponse.withCode(.code200_OK, version: httpVersion, mimeType: responseMimeType, body: responsePayload!)
