...
public final class Forwarder: SwifterSockets.Connection {
    ...
    public override func receiverClosed() {
        closeForwarder()
    }
    
    public override func receiverLoop() -> Bool {
        let reply = Response()
    ...
    public override func receiverError(_ message: String) {
        Log.atError?.log(message, id: Int(client!.interface!.logId), type: "Forwarder")
        closeForwarder()
    }
    
    public override func processReceivedData(_ buffer: UnsafeBufferPointer<UInt8>) -> Bool {
