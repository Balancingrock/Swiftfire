...
public func sendEmail(_ mail: String, domainName: String) {
    ...
    do {
        ...
        Log.atDebug?.log("Sendmail for \(mail)")
        ...
        let timeoutAfter = DispatchTime(uptimeNanoseconds: DispatchTime.now().uptimeNanoseconds + UInt64(10) * 1000000)
        DispatchQueue.global().asyncAfter(deadline: timeoutAfter) {
            [weak process] in
            Log.atDebug?.log("SendmailTimeout expired, process \(process != nil ? "is still running" : "has exited already")")
            process?.terminate()
        }
