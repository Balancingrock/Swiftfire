...
final class HttpConnectionPool: NSObject {
    ...
    /// Allocates an HTTP Connection if it becomes available within the timeout period.
    ///
    /// - Parameter timeout: An integer specifying the timout in seconds
    ///
    /// - Returns: Either an available HTTP connection or a nil after the timeout expires.
    
    func allocateOrTimeout(_ timeout: Int) -> HttpConnection? {
        ...
        while true {
            ...
            // Try again in 1 second unless the specified timeout expires
            ...
            if loopCount > (timeout / 2) {
                ...
                let message = "Connection objects are no longer available (waited for \(loopCount) seconds, trying to force free the oldest...)"
                log.atLevelEmergency(id: 0, source: #file.source(#function, #line), message: message)
                
                // Try to force free the oldest connection object
